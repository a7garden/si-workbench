...
`.base` 파일은 Obsidian Bases의 뷰 정의입니다. 개념·사업·개선·일지를 표로 훑어보는 용도이며, **지식은 항상 노트에 있습니다** — `.base`를 전부 지워도 잃는 것은 보기 방식뿐입니다. 시작 화면으로 `대시보드.md`를 열려면 커뮤니티 플러그인 **Homepage**를 설치하세요 (init-vault가 설정을 잡아줍니다).
...
발견한 문제를 `사업/<사업명>/개선/<화면단위>/문제목록 - <화면단위>.md` 에 편하게 적어두고 `/si-workbench:improve` 를 실행하면:
